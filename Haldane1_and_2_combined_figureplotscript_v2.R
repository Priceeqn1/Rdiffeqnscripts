...
# Small-U grid for the multiclass convergence test shown in Figure 1A.
U_grid <- c(
  0.04,
  0.02,
  0.01,
...
feedback_ratio <- function(f, pars) {
  ...
  w_rho <- sum(
    pars$zeta_rho^classes * f
  )
  
  w_gamma <- sum(
    pars$zeta_gamma^classes * f
  )
  
  if (!is.finite(w_rho) || w_rho <= 0) {
    stop(
      "Mean relative fecundity became nonpositive or nonfinite."
    )
  }
...
make_multiclass_rhs <- function(U, pars, nu) {
  ...
  if (length(nu) != n) {
    stop(
      "nu must have length n, with nu[i] representing repair i -> i-1."
    )
  }
...
equilibrate_multiclass <- function(U, pars, nu, settings) {
  ...
  for (chunk_id in seq_len(settings$max_chunks)) {
    ...
    population_error <- abs(
      sum(state) - 1
    )
    
    if (
      derivative_norm <
      settings$derivative_tolerance &&
      population_error <
      1e-9
    ) {
      converged <- TRUE
  ...
  if (!converged) {
    warning(
      sprintf(
        paste0(
          "Equilibrium convergence criterion not met ",
          "for U = %.8g; max |df/dtau| = %.3e; ",
          "|sum(f)-1| = %.3e"
        ),
        U,
        derivative_norm,
...
mutation_load <- function(f, pars) {
  ...
  s <- pars$epsilon * (
    pars$zeta_gamma^classes -
      pars$zeta_rho^classes
  )
...
# Do not permit a failed equilibrium calculation to enter
# the reported regression.
if (!all(panel_A$converged)) {
  stop(
    paste0(
      "At least one Panel A equilibrium failed ",
...
slope_fit <- stats::lm(
  log(panel_A$absolute_error) ~
    log(panel_A$U)
)

panel_A_slope <- unname(
  stats::coef(slope_fit)[2L]
)
...
anchor_index <- which.min(
  panel_A$U
)

reference_constant <-
  panel_A$absolute_error[anchor_index] /
  panel_A$U[anchor_index]^2
...
run_all_class_repair_check <- function(nu_value) {
  ...
  rows <- lapply(
    ...
    function(U) {
      ...
      # Corollary 1 depends at leading order only on nu_1.
      # Here every damaged class has the same repair rate,
      # so nu_1 = nu_value.
      L_asymptotic <-
        pars$epsilon *
        U *
  ...
  # Do not permit a failed equilibrium calculation to enter
  # the reported regression.
  if (!all(dat$converged)) {
    stop(
      sprintf(
        paste0(
          "At least one all-class repair equilibrium ",
          "failed to converge for nu = %.8g."
        ),
        nu_value
      )
    )
...
# Integrate the untransformed exact binary equation, Eq. (19) of the
# manuscript, at the no-repair threshold U = U_c.
#
#   dx/dtau =
#     epsilon * (1 - x) * [U * (1 + b*x) - d*x] / (1 - a*x)
#     - nu*x
#
# Here U = U_c, nu = 0, and x(0) = 0.

a <- 1 - pars$zeta_rho
b <- pars$zeta_gamma - 1
d_binary <- pars$zeta_gamma - pars$zeta_rho


threshold_x_rhs <- function(time, state, parameters) {
  x <- state[1L]
...
binary_equilibrium <- function(U, nu, pars) {
  ...
  if (Delta < -1e-12) {
    stop(
      "Binary discriminant became negative beyond roundoff."
    )
  }
...
# ------------------------------------------------------------------------------
# 7. Write numerical tables and reproducibility information
# ------------------------------------------------------------------------------
...
summary_lines <- c(
  ...
  sprintf(
    paste0(
      "Largest equilibrium derivative ",
      "residual in Panel A = %.3e"
    ),
    max(
      panel_A$derivative_norm
    )
  ),
  sprintf(
    paste0(
      "Largest population-conservation ",
      "residual in Panel A = %.3e"
    ),
    max(
      panel_A$population_error
    )
  ),
  sprintf(
    paste0(
      "Final normalized threshold ",
      "dynamic quantity = %.8f"
    ),
    tail(
      panel_B$normalized_dynamic_scaling,
  ...
  sprintf(
    paste0(
      "Smallest-nu normalized repair ",
      "quantity = %.8f"
    ),
    panel_C$normalized_repair_scaling[
      which.min(panel_C$nu)
    ]
  ),
  sprintf(
    paste0(
      "Panel-A fit range: %.6g <= U <= ",
      "%.6g (%d points)"
    ),
    min(panel_A$U),
    max(panel_A$U),
  ...
  sprintf(
    paste0(
      "All-class repair slope, nu_i = 0.1 ",
      "for all i >= 1 = %.6f"
    ),
    all_class_slopes[1L]
  ),
  sprintf(
    paste0(
      "All-class repair slope, nu_i = 0.5 ",
      "for all i >= 1 = %.6f"
    ),
    all_class_slopes[2L]
  ),
  sprintf(
    paste0(
      "Largest equilibrium derivative residual ",
      "in all-class check = %.3e"
    ),
    max(
      all_class_repair$derivative_norm
    )
  ),
  sprintf(
    paste0(
      "Largest population-conservation residual ",
      "in all-class check = %.3e"
    ),
    max(
      all_class_repair$population_error
    )
  ),
  "",
  sprintf(
...
draw_validation_panel <- function(
    device = c(
      "pdf",
      "png"
    )
  ...
  graphics::plot(
    ...
    main =
      "A. Multiclass rare-mutation error"
  )
...
pdf_file <- draw_validation_panel(
  "pdf"
)

png_file <- draw_validation_panel(
  "png"
)
